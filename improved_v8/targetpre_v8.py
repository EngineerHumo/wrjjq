...
class RealTarget:
    ...
    def __init__(self, ID, priority, start_belief_pos, initial_v, initial_phi, v_range=None, phi_range=None):
        ...
        self.phi_range_wrap = self.phi_range[1] < self.phi_range[0]

    def _phi_in_range(self, angle):
        if not self.phi_range_wrap:
            return self.phi_range[0] <= angle <= self.phi_range[1]
        return angle >= self.phi_range[0] or angle <= self.phi_range[1]

    def _closest_phi_boundary(self, angle):
        def angular_distance(a, b):
            return abs(normalize_angle(a - b))

        start, end = self.phi_range
        dist_start = angular_distance(angle, start)
        dist_end = angular_distance(angle, end)
        return start if dist_start <= dist_end else end

    def _clamp_phi(self, angle):
        if self._phi_in_range(angle):
            return angle
        return self._closest_phi_boundary(angle)
    ...
    def step_forward(self):
        ...
        if abs(self.state[5]) > 1e-5:
            ...
            self.state[4] = self._clamp_phi(self.state[4])
