...
class Target:
    ...
    def step(self):
        ...
        if self.x < 0 or self.x > Config.MAP_SIZE: self.phi = np.pi - self.phi
        if self.y < 0 or self.y > Config.MAP_SIZE: self.phi = -self.phi
...
class UAVSwarmEnv(gym.Env):
    def __init__(self):
        ...
        obs_dim = 4 + 2 + 3 + (Config.N_UAV - 1) * 2 + 1
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(obs_dim,), dtype=np.float32)

        self.n_agents = Config.N_UAV
        self.agents = []
        self.targets = []
    ...
    def reset(self, seed=None, options=None):
        ...
        # 初始化目标
        self.targets = [Target() for _ in range(Config.N_TARGET)]
    ...
    def step(self, actions):
        ...
        for i, agent in enumerate(self.agents):
            ...
            r_step += Config.W_SMOOTH * (acc_norm + omega_norm)
            ...
            if dist_to_obs < Config.SAFE_DIST_OBS:
                if dist_to_obs <= 0:  # 撞击
                    r_step += Config.W_OBS * 2
                else:  # 警告区
                    r_step += Config.W_OBS * (1 - dist_to_obs / Config.SAFE_DIST_OBS)
    ...
    def _get_all_obs(self):
        ...
        for i, agent in enumerate(self.agents):
            ...
            # 2. 局部障碍物 (最近距离 + 密度) [cite: 298]
            dists = [np.sqrt((agent['x'] - ox) ** 2 + (agent['y'] - oy) ** 2) for ox, oy, _ in self.obstacles]
            min_dist = min(dists) if dists else Config.SENSOR_RANGE
            obs_info = [min(min_dist, Config.SENSOR_RANGE) / Config.SENSOR_RANGE, 0.0]  # 简化密度

            # 3. 局部概率特征 (最大值, 均值, 梯度方向) [cite: 298]
            # 提取局部网格
            ...
            # 拼接
            obs = np.concatenate([own_state, obs_info, local_prob, neighbor_info, [cov_val]])
