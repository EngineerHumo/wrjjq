...
def is_angle_in_range(angle, start, end, wraps):
    """
    判断角度是否在给定区间内。wraps=True 表示区间跨越 ±pi。
    """
    if wraps:
...
def clamp_angle_to_range(angle, start, end, wraps):
    """
    将角度限制在给定区间内，必要时钳制到最近边界。
    """
    angle = normalize_angle(angle)
    if is_angle_in_range(angle, start, end, wraps):
        return angle
    if wraps:
        dist_to_start = angular_distance(angle, start)
        dist_to_end = angular_distance(angle, end)
        return start if dist_to_start <= dist_to_end else end
    return np.clip(angle, start, end)
...
class RealTarget:
    ...
    def __init__(self, ID, priority, start_belief_pos, initial_v, initial_phi, v_range=None, phi_range=None):
        ...
        self.phi_range_wraps = phi_start > phi_end
        self.phi_range = (phi_start, phi_end)

    def step_forward(self):
        """真实目标步进"""
        # 情况 A: 碰到了上边界 ，且当前正在向更大角度转 (w > 0)
        phi_start, phi_end = self.phi_range

        # 情况 A: 碰到了上边界 ，且当前正在向更大角度转 (w > 0)
        if not self.phi_range_wraps:
            if self.state[4] >= phi_end and self.state[5] > 0:
                self.state[5] = -self.turn_rate  # 强制改为负转速（向右转回去）
            # 情况 B: 碰到了下边界，且当前正在向更小角度转 (w < 0)
            elif self.state[4] <= phi_start and self.state[5] < 0:
                self.state[5] = self.turn_rate  # 强制改为正转速（向左转回去）
        else:
            # wrap 情况下，区间跨越 ±pi，边界判断需要反向处理
            if self.state[4] <= phi_end and self.state[5] > 0:
        ...
        if abs(self.state[5]) > 1e-5:
            ...
            self.state[4] = clamp_angle_to_range(
                self.state[4],
                phi_start,
                phi_end,
                self.phi_range_wraps
            )
