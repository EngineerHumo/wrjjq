...
class UAVAgent:
    ...
    @staticmethod
    def _compute_uncertainty_gradient(center, map_size, target_predictors, radius, delta=30.0):
        if not target_predictors:
            return np.zeros(2)
        ...
        def clamp_point(point):
            return np.array([
                np.clip(point[0], 0.0, N),
                np.clip(point[1], 0.0, M)
            ])
        ...
        def local_entropy_at(pos):
            entropy_val = 0.0
            for predictor in target_predictors:
                weight = float(getattr(predictor, "priority", 1.0))
                entropy_val += weight * predictor.get_local_entropy(pos, radius)
            return entropy_val

        e_pos_x = local_entropy_at(clamp_point(center + offsets[0]))
        e_neg_x = local_entropy_at(clamp_point(center + offsets[1]))
        e_pos_y = local_entropy_at(clamp_point(center + offsets[2]))
        e_neg_y = local_entropy_at(clamp_point(center + offsets[3]))

        grad_x = (e_pos_x - e_neg_x) / (2.0 * delta)
        grad_y = (e_pos_y - e_neg_y) / (2.0 * delta)
    ...
    def calculate_reward(
        ...
        # 2.5 不确定区域引导
        grad = self._compute_uncertainty_gradient(
            center=self.pos,
            map_size=map_size,
        ...
        move_vec = self.pos - prev_pos
        move_norm = np.linalg.norm(move_vec)
        grad_norm = np.linalg.norm(grad)
        if move_norm > 1e-6 and grad_norm > 1e-6:
            move_dir = move_vec / move_norm
            grad_dir = grad / grad_norm
            alignment = float(np.clip(np.dot(move_dir, grad_dir), -1.0, 1.0))
            r_uncertainty = 1.2 * max(0.0, alignment)
        else:
            r_uncertainty = 0.0
        ...
        if out_of_bounds:
            ...
            if self.distance_to_bounds >= self.prev_distance_to_bounds:
                distance_increase = self.distance_to_bounds - self.prev_distance_to_bounds
                r_boundary += -0.2 - 0.1 * distance_increase
            else:
                distance_decrease = self.prev_distance_to_bounds - self.distance_to_bounds
                r_boundary += 0.05 * distance_decrease
        else:
