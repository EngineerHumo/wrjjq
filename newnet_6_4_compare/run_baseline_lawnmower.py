...
from baselines import LawnmowerController
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="./compare_results/lawnmower")
    ...
    for target_count in target_counts:
        ...
        save_metrics(result_dir, {"Lawnmower": metrics})

        save_trajectory_plots(
            lambda: UAVSwarmEnv(n_uav=args.n_uav, n_target=target_count, use_pf=True, use_pf_obs=True),
            lambda env: EnvControllerPolicy(LawnmowerController(env)),
            sampled_seeds,
            args.max_steps,
            traj_dir,
        )
