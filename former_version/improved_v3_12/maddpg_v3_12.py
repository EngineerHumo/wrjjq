...
class UAVAgent:
    def __init__(self, uav_id, initial_pos, initial_v, initial_phi, step,
        ...
        self.lost_streak = 0
        self.last_tracking_event = "none"
        self.prev_tracking_distance = None
    ...
    def update_tracking_state(self, detected_targets, nearest_target):
        ...
        self.last_tracking_event = event
        return event
    ...
    def get_observation(self, map_size, obstacles_map, all_uavs, target_predictors):
        ...
        if self.tracking_target_id is not None and target_predictors:
            lock_target_id_norm = self.tracking_target_id / len(target_predictors)
        else:
            lock_target_id_norm = -1.0
        detect_streak_norm = min(self.detect_streak, 20) / 20.0
        lost_streak_norm = self.lost_streak / LOST_TOL
        rel_dx, rel_dy = 0.0, 0.0
        if self.tracking_target_id is not None and target_predictors:
            if 0 <= self.tracking_target_id < len(target_predictors):
                estimate_pos = target_predictors[self.tracking_target_id].state_si[0:2]
                rel_dx = (estimate_pos[0] - x) / self.detect_radius
                rel_dy = (estimate_pos[1] - y) / self.detect_radius
        ...
        # 拼接
        final_obs = np.concatenate((obs, global_belief, uncertainty_sectors, tracking_state))
    ...
    def calculate_reward(self, prev_entropy, curr_entropy, detected_any, detected_targets, nearest_target,
        ...
        if detected_any:
            ...
            r_uncertainty = 0.2 * curr_entropy
            if self.tracking_target_id is not None:
                r_uncertainty *= UNCERT_TRACK_SCALE

        r_track = 0.0
        if self.last_tracking_event == "acquire":
            r_track += ACQUIRE_REWARD
        elif self.last_tracking_event == "reacquire":
        ...
        if self.tracking_target_id is not None and predictors:
            if 0 <= self.tracking_target_id < len(predictors):
                estimate_pos = predictors[self.tracking_target_id].state_si[0:2]
                dist = np.linalg.norm(self.pos - estimate_pos)
                dist_norm = min(dist / self.detect_radius, 1.0)
                r_track -= TRACK_DIST_W * dist_norm
                if self.prev_tracking_target_id != self.tracking_target_id or self.prev_tracking_distance is None:
        ...
        dx = max(0.0, -x, x - N)
        dy = max(0.0, -y, y - M)
        self.distance_to_bounds = float(np.hypot(dx, dy))
        out_of_bounds = self.distance_to_bounds > 0.0

        if out_of_bounds:
            r_action = 0.0
            delta_dist = self.distance_to_bounds - self.prev_distance_to_bounds
            if self.distance_to_bounds >= self.prev_distance_to_bounds:
                r_out = -0.2 - 0.1 * delta_dist
            else:
                r_out = 0.05 * (self.prev_distance_to_bounds - self.distance_to_bounds)
