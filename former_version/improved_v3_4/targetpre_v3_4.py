...
def angle_in_range(angle, start, end):
    """
    判断角度是否落在[start, end]区间内（支持跨越 ±pi 的环绕区间）
    """
    if start <= end:
...
def clamp_angle_to_range(angle, start, end):
    """
    将角度夹到[start, end]区间（支持跨越 ±pi 的环绕区间）
    """
    if angle_in_range(angle, start, end):
        return angle
    dist_to_start = abs(normalize_angle(angle - start))
    dist_to_end = abs(normalize_angle(angle - end))
...
class RealTarget:
    ...
    def step_forward(self):
        ...
        if abs(self.state[5]) > 1e-5:
            omega_dt = self.state[5] * dt
            sin_wt = np.sin(omega_dt)
            cos_wt = np.cos(omega_dt)
            vx_prev = self.state[2]
            vy_prev = self.state[3]

            next_x = self.state[0] + (vx_prev * sin_wt - vy_prev * (1 - cos_wt)) / self.state[5]
            next_y = self.state[1] + (vx_prev * (1 - cos_wt) + vy_prev * sin_wt) / self.state[5]
            next_vx = vx_prev * cos_wt - vy_prev * sin_wt
            next_vy = vx_prev * sin_wt + vy_prev * cos_wt
            next_phi = normalize_angle(self.state[4] + self.state[5] * dt)
        else:
            next_x = self.state[0] + self.state[2] * dt
            next_y = self.state[1] + self.state[3] * dt
            next_vx = self.state[2]
            next_vy = self.state[3]
            next_phi = normalize_angle(self.state[4])

        speed = np.sqrt(next_vx ** 2 + next_vy ** 2)
        speed = np.clip(speed, self.v_range[0], self.v_range[1])
        next_vx = speed * np.cos(next_phi)
        next_vy = speed * np.sin(next_phi)

        self.state[0] = next_x
        self.state[1] = next_y
        self.state[2] = next_vx
        self.state[3] = next_vy
        self.state[4] = next_phi

        if self.state[0] < 0:
            self.state[0] = -self.state[0]
            self.state[2] = -self.state[2]
            self.state[4] = np.arctan2(self.state[3], self.state[2])
        elif self.state[0] > N:
            self.state[0] = 2 * N - self.state[0]
            self.state[2] = -self.state[2]
            self.state[4] = np.arctan2(self.state[3], self.state[2])
...
class TargetPredictor:
    ...
    def get_local_entropy_gradient(self, center, radius, delta=10.0):
        """
        估计局部熵梯度（用于引导无人机朝高不确定区域移动）。
        """
        ...
        offset_x = np.array([delta, 0.0])
        offset_y = np.array([0.0, delta])

        h_x1 = self.get_local_entropy(center + offset_x, radius)
        h_x2 = self.get_local_entropy(center - offset_x, radius)
        h_y1 = self.get_local_entropy(center + offset_y, radius)
        h_y2 = self.get_local_entropy(center - offset_y, radius)

        grad_x = (h_x1 - h_x2) / (2.0 * delta)
        grad_y = (h_y1 - h_y2) / (2.0 * delta)
