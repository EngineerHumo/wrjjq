...
class RealTarget:
    ...
    def __init__(self, ID, priority, start_belief_pos, initial_v, initial_phi, v_range=None, phi_range=None):
        ...
        # v9 解开角度锁，phi_range 保留以兼容旧配置
        self.phi_range = phi_range

    def step_forward(self):
        """真实目标步进"""
        # 解开角度锁：允许目标向任何方向移动，仅保留随机机动性
...
class TargetPredictor:
    ...
    def __init__(self, map_size, obstacles_map, v_range, theta_range, init_pos, num_particles=1000,
        ...
        :param init_pos: 初始位置
        :param theta_range: 兼容旧接口，v9 中不再限制角度范围
        :param num_particles:
        :param ID:
        :param priority:
        :param dt:
        ...
        self.th_min, self.th_max = (-np.pi, np.pi)
        ...
        # 过程噪声标准差 (基准值)
        # [x, y, vx, vy, omega] 的扰动
        self.Q_std = np.array([0.08, 0.08, 0.35, 0.35, 0.008])
        ...
        # 记录粒子云收缩状态：用于全局信念奖励的控制
        self.collapse_active = False
        self.last_detection_spread = None
        self.recover_spread_ratio = 1.5
    ...
    def step_update(self, z, innovation_norm, det_res):
        ...
        self._update_collapse_state(z)
        return self._generate_grid()

    def _compute_particle_spread(self):
        """
        计算粒子云扩散程度（加权平均距离）
        """
        if self.num_particles == 0:
            return 0.0
        center = self.estimate()[0:2]
        d_vec = self.particles[:, 0:2] - center
        dists = np.linalg.norm(d_vec, axis=1)
        return float(np.average(dists, weights=self.weights))

    def _update_collapse_state(self, measurement):
        """
        更新粒子云收缩/扩散状态。
        - 若被探测到：标记为收缩状态。
        - 若未探测到：当粒子云扩散到一定程度后，解除收缩状态。
        """
        if measurement is not None:
            self.collapse_active = True
            self.last_detection_spread = self._compute_particle_spread()
            return

        if not self.collapse_active:
            return

        current_spread = self._compute_particle_spread()
        if self.last_detection_spread is None:
            self.last_detection_spread = current_spread
            return

        if current_spread >= self.last_detection_spread * self.recover_spread_ratio:
            self.collapse_active = False
