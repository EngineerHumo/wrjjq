...
import maddpg_v7 as RL
import targetpre_v7 as tp
...
def save_reward_history(output_dir, reward_history, noise_history):
    ...
    plot_training_curve(output_dir, reward_history)


def plot_training_curve(output_dir, reward_history):
    ensure_dir(output_dir)
...
def plot_trajectories(output_dir, map_size, obs_map, uav_trajectories, target_trajectories, detection_points, eval_tag):
    ...
    ax = plt.gca()
    boundary = Rectangle((0, 0), map_size[1], map_size[0], fill=False, edgecolor="gray", linewidth=2)
...
def train_with_improvements():
    ...
    min_noise = 0.05
    noise_decay = 0.996
    noise_reset_after = 15000
    noise_reset_interval = 100
    noise_decay_post = 0.9
    eval_interval = 200
    ...
    for episode in range(MAX_EPISODES):
        ...
        if episode + 1 < noise_reset_after:
            noise_std = max(min_noise, noise_std * noise_decay)
        else:
            if episode + 1 == noise_reset_after:
                noise_std = min_noise
            if (episode + 1 - noise_reset_after) % noise_reset_interval == 0:
                noise_std = noise_std * noise_decay_post
        ...
        if (episode + 1) % 200 == 0:
            plot_training_curve(output_root, reward_history)
