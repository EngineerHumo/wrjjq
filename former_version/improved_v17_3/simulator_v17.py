...
import maddpg_v17 as RL
import targetpre_v17 as tp
...
def init_predictors_targets(map_size, obstacles, tarcfgs, rng=None):
    predictors = []
    real_targets = []
    if rng is None:
    ...
    for cfg in tarcfgs:
        ...
        if initial_v is None:
            initial_v = float(rng.uniform(v_range[0], v_range[1]))
...
def select_target_configs(base_tarcfgs, target_ids=None, max_targets=4):
    if target_ids is None:
        target_ids = [cfg["ID"] for cfg in base_tarcfgs]
    if len(target_ids) > max_targets:
        raise ValueError(f"target_ids length {len(target_ids)} exceeds max_targets {max_targets}.")
    selected = [cfg for cfg in base_tarcfgs if cfg["ID"] in target_ids]
    if not selected:
        raise ValueError("No target configs selected. Please check target_ids.")
    return selected
...
def train_with_improvements():
    ...
    # 定义目标配置
    tarcfgs = [
        {"ID": 1, "pos": (500.0, 1000.0), "priority": 1, "v_range": (8.0, 30.0),
         "theta_range": (0.0, 2 * np.pi), "initial_v": None, "initial_phi": 45},
        {"ID": 2, "pos": (1000.0, 500.0), "priority": 1, "v_range": (8.0, 30.0),
         "theta_range": (0.0, 2 * np.pi), "initial_v": None, "initial_phi": 135},
        {"ID": 3, "pos": (1500.0, 1000.0), "priority": 1, "v_range": (8.0, 30.0),
         "theta_range": (0.0, 2 * np.pi), "initial_v": None, "initial_phi": 225},
        {"ID": 4, "pos": (1000.0, 1500.0), "priority": 1, "v_range": (8.0, 30.0),
         "theta_range": (0.0, 2 * np.pi), "initial_v": None, "initial_phi": 315}
    ]
    ...
    # 无人机参数配置
    uav_configs = [
        {"id": 1, "pos": [400, 400], "phi": 45, "initial_v": 20},
        {"id": 2, "pos": [1600, 1600], "phi": 225, "initial_v": 20},
        {"id": 3, "pos": [400, 1600], "phi": 315, "initial_v": 20},
        {"id": 4, "pos": [1600, 400], "phi": 135, "initial_v": 20},
        {"id": 5, "pos": [1000, 1000], "phi": 0, "initial_v": 20}
    ]
    ...
    # 训练主循环
    target_ids = [1, 2, 3]
    tarcfgs_selected = select_target_configs(tarcfgs, target_ids=target_ids, max_targets=4)

    print("开始训练 Improved MADDPG")
    for episode in range(MAX_EPISODES):
        predictors, real_targets = init_predictors_targets(map_size, obstacles, tarcfgs_selected)
        ...
        if (episode + 1) % eval_interval == 0:
            ...
            eval_dir, episode + 1, uav_list, uav_configs, map_size, obs_map, obstacles, tarcfgs_selected, MAX_STEPS
        )
