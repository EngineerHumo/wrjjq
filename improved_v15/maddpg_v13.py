...
class UAVAgent:
    ...
    def state_update(self, action, map_size, obstacles_map):
        ...
        if next_x < 0 or next_x > max_x:
            ...
            next_x = np.clip(next_x, 0.0, max_x)
            self.phi = np.pi - self.phi  # 左右边界镜像反弹

        if next_y < 0 or next_y > max_y:
            self.hit_boundary = True
            next_y = np.clip(next_y, 0.0, max_y)
            self.phi = -self.phi         # 上下边界镜像反弹
    ...
    def calculate_reward(self, prev_entropy, curr_entropy, is_detected, action, map_size, obstacles_map, all_uavs, target_predictors=None):
        """
        最小化熵 + 探测保持 + 安全约束
        """
        INFO_COEF = 2.0
        ...
        TURN_COEF = 0.02
        EDGE_MARGIN = 250.0
        EDGE_PENALTY_MAX = 0.5
        HIT_BOUNDARY_PENALTY = 0.5
        OOB_STEP_PENALTY = 1.0
        OOB_TERMINATE_STEPS = 8
        OOB_TERMINATE_PENALTY = 10.0
        ...
        # 1) 信息增益（熵减，密集但别太大）
        delta_h = float(prev_entropy - curr_entropy)
        delta_h = np.clip(delta_h, -1.0, 1.0)
        r_info = INFO_COEF * delta_h

        # 2) 探测奖励：ACQUIRE / KEEP / LOST / REACQUIRE（密集可学）
        r_track = 0.0
        if is_detected and (not self.prev_detected):
            if self.steps_since_detect > 0:
                r_track += REACQUIRE_REWARD
            else:
                r_track += ACQUIRE_REWARD
            self.steps_since_detect = 0
        elif is_detected and self.prev_detected:
            r_track += KEEP_REWARD
            self.steps_since_detect = 0
        elif (not is_detected) and self.prev_detected:
        ...
        # 3) 朝“全局信念方向”前进的密集引导（避免只会原地转圈）
        r_align = 0.0
        if target_predictors is not None:
            g_dx, g_dy = self._compute_global_belief_direction(self.pos, map_size, target_predictors)
            v_dx, v_dy = np.cos(self.phi), np.sin(self.phi)
            dot = float(np.clip(v_dx * g_dx + v_dy * g_dy, -1.0, 1.0))
            r_align = ALIGN_COEF * dot

        # 4) 动作惩罚 + 额外转向惩罚（抑制无意义盘旋，但不要太大）
        r_action = -0.05 * float(np.sum(action ** 2))
        r_turn = -TURN_COEF * float(action[1] ** 2)

        # 5) 贴边惩罚（密集，避免“沿边转圈/撞边”）
        dist_to_edge = min(x, (N - x), y, (M - y))
        edge_ratio = max(0.0, (EDGE_MARGIN - dist_to_edge) / EDGE_MARGIN)
        r_edge = -EDGE_PENALTY_MAX * edge_ratio

        # 6) 触边惩罚（本步发生反弹/夹紧）
        r_hit = -HIT_BOUNDARY_PENALTY if getattr(self, "hit_boundary", False) else 0.0

        # 7) 越界强惩罚 + 可提前终止（理论上很少触发）
        out_of_bounds = (x < 0) or (x > N) or (y < 0) or (y > M)
        r_oob = 0.0
        if out_of_bounds:
            r_oob -= OOB_STEP_PENALTY
            self.steps_out_of_bounds += 1
            if self.steps_out_of_bounds >= OOB_TERMINATE_STEPS:
                r_oob -= OOB_TERMINATE_PENALTY
                self.force_done = True
        else:
            self.steps_out_of_bounds = 0

        # 8) 保留你原来的：障碍物碰撞惩罚 + 机间避碰惩罚（r_collision）
        r_collision = 0.0
        ...
        # 9) 更新历史
        self.prev_detected = bool(is_detected)

        total_reward = r_info + r_track + r_align + r_action + r_turn + r_edge + r_hit + r_oob + r_collision
        return total_reward
