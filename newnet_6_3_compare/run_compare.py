...
def main():
    ...
    env = UAVSwarmEnv(n_uav=args.n_uav, n_target=args.n_target, use_pf=True, use_pf_obs=True)
    ...
    env = UAVSwarmEnv(n_uav=args.n_uav, n_target=args.n_target, use_pf=False, use_pf_obs=False)
    obs_dim = env.observation_space.shape[0]
    act_dim = env.action_space.shape[0]
    global_obs_dim = obs_dim * env.n_agents
