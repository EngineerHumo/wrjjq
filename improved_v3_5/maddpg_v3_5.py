...
class UAVAgent:
    ...
    def state_update(self, action, map_size, obstacles_map):
        ...
        self.last_move = np.array([dx, dy], dtype=float)
    ...
    def calculate_reward(self, prev_entropy, curr_entropy, detected_any, detected_targets, action, map_size,
        ...
        if out_of_bounds:
            ...
            if self.distance_to_bounds >= self.prev_distance_to_bounds:
                delta = self.distance_to_bounds - self.prev_distance_to_bounds
                r_boundary -= 0.2 + 0.1 * delta
            else:
                delta = self.prev_distance_to_bounds - self.distance_to_bounds
                r_boundary += 0.05 * delta
        else:
            self.steps_out_of_bounds = 0
        ...
        if target_predictors:
            ...
            for predictor in target_predictors:
                weight = float(getattr(predictor, "priority", 1.0))
                grad += predictor.get_local_entropy_gradient(self.pos, self.detect_radius) * weight
            grad_norm = np.linalg.norm(grad)
            move_norm = np.linalg.norm(self.last_move)
            if grad_norm > 1e-6 and move_norm > 1e-6:
                grad_dir = grad / grad_norm
                move_dir = self.last_move / move_norm
        ...
        if self.tracking_target_id is not None:
            ...
            if target_predictors and self.tracking_target_id < len(target_predictors):
            ...
            for other in all_uavs:
                if other is self:
                    continue
                if other.tracking_target_id == self.tracking_target_id:
                    same_trackers += 1
            if same_trackers > 0:
                r_track -= multi_track_penalty * same_trackers
