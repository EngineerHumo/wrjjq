...
import maddpg_v3_5 as RL
import targetpre_v3_5 as tp
...
def save_tracking_metrics(output_dir, metrics_history):
    ...
    json_path = os.path.join(output_dir, "tracking_metrics.json")
    with open(csv_path, "w", encoding="utf-8") as f:
        f.write(
            "episode,avg_detect_streak,avg_lost_streak,lock_count,lock_time_ratio,target_covered_ratio\n"
        )
        for entry in metrics_history:
            f.write(
                "{episode},{avg_detect_streak:.4f},{avg_lost_streak:.4f},{lock_count:.2f},{lock_time_ratio:.4f},"
                "{target_covered_ratio:.4f}\n".format(**entry)
            )
...
def train_with_improvements():
    ...
    base_target_config = [
        {"ID": 1, "pos": (500.0, 500.0), "priority": 1, "v_range": (15, 60),
         "theta_range": (np.radians(15), np.radians(60))},
        {"ID": 2, "pos": (1500.0, 1500.0), "priority": 1, "v_range": (15, 60),
         "theta_range": (np.radians(225), np.radians(270))}
    ]
    ...
    min_noise = 0.05
    noise_decay = 0.996
    schedule_switch_episode = 13000
    schedule_decay_interval = 100
    schedule_decay_ratio = 0.9
    eval_interval = 200
    ...
    for episode in range(MAX_EPISODES):
        # === 场景随机化 ===
        tarcfgs = []
        for base in base_target_config:
            rand_v = np.random.uniform(base["v_range"][0], base["v_range"][1])

            rand_x = base["pos"][0] + np.random.uniform(-30, 30)
            rand_y = base["pos"][1] + np.random.uniform(-30, 30)

            new_theta_range = (-np.pi, np.pi)
            rand_phi_deg = np.random.uniform(-180, 180)

            cfg = base.copy()
            cfg["theta_range"] = new_theta_range
            cfg["initial_v"] = rand_v
            cfg["initial_phi"] = rand_phi_deg
            cfg["pos"] = (rand_x, rand_y)
            tarcfgs.append(cfg)
        ...
        episode_reward = 0
        detect_streak_sum = np.zeros(num_uavs, dtype=float)
        detect_streak_samples = np.zeros(num_uavs, dtype=float)
        lost_streak_sum = np.zeros(num_uavs, dtype=float)
        lost_streak_samples = np.zeros(num_uavs, dtype=float)
        lock_count = np.zeros(num_uavs, dtype=float)
        lock_steps = np.zeros(num_uavs, dtype=float)
        target_covered_steps = np.zeros(len(predictors), dtype=float)
        ...
        for t in range(MAX_STEPS):
            ...
            for i, uav in enumerate(uav_list):
                ...
                prev_tracking_id = uav.tracking_target_id
                ...
                if prev_tracking_id is None and uav.tracking_target_id is not None:
                    lock_count[i] += 1

                if uav.tracking_target_id is not None:
                    lock_steps[i] += 1
                    if uav.tracking_target_id in uav_detected_targets[i]:
                        detect_streak_sum[i] += uav.detect_streak
                        detect_streak_samples[i] += 1
                    else:
                        lost_streak_sum[i] += uav.lost_streak
                        lost_streak_samples[i] += 1
            ...
            for idx, detected in enumerate(target_detected_any):
                if detected:
                    target_covered_steps[idx] += 1

        if episode < schedule_switch_episode:
            noise_std = max(min_noise, noise_std * noise_decay)
        else:
            if episode == schedule_switch_episode:
                noise_std = min_noise
            elif (episode - schedule_switch_episode) % schedule_decay_interval == 0:
                noise_std = noise_std * schedule_decay_ratio
        ...
        avg_detect_streak = float(np.mean(
            detect_streak_sum / np.maximum(1.0, detect_streak_samples)
        ))
        avg_lost_streak = float(np.mean(
            lost_streak_sum / np.maximum(1.0, lost_streak_samples)
        ))
        lock_time_ratio = float(np.sum(lock_steps) / (len(uav_list) * MAX_STEPS))
        target_covered_ratio = float(np.mean(target_covered_steps / MAX_STEPS))

        metrics_history.append({
            "episode": episode + 1,
            "avg_detect_streak": avg_detect_streak,
            "avg_lost_streak": avg_lost_streak,
            "lock_count": float(np.mean(lock_count)),
            "lock_time_ratio": lock_time_ratio,
            "target_covered_ratio": target_covered_ratio,
            "per_uav_detect_streak": (detect_streak_sum / np.maximum(1.0, detect_streak_samples)).tolist(),
            "per_uav_lost_streak": (lost_streak_sum / np.maximum(1.0, lost_streak_samples)).tolist(),
            "per_uav_lock_count": lock_count.tolist(),
            "per_uav_lock_time_ratio": (lock_steps / MAX_STEPS).tolist(),
            "per_target_covered_ratio": (target_covered_steps / MAX_STEPS).tolist()
        })
