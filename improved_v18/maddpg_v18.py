...
class UAVAgent:
    ...
    def _compute_global_belief_direction(agent_pos, map_size, target_predictors):
        ...
        if nearest_predictor is None:
            return 0.0, 0.0

        if getattr(nearest_predictor, "collapse_active", False):
            return 0.0, 0.0

        global_point = nearest_predictor.estimate()[0:2]
    ...
    def calculate_reward(self, prev_entropy, curr_entropy, is_detected, action, map_size, obstacles_map, all_uavs):
        ...
        total_reward = r_info + r_detect + r_action + r_collision + r_boundary
        return total_reward
